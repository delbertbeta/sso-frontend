<template>
  <div class="face-uploader-wrapper" :class="{ editing }" @click="handleClick" :style="style">
    <t-avatar v-if="editing && imageBlobUrl" :size="size" :image="imageBlobUrl" />
    <self-face v-else :size="size" />
...
<style scoped>
.face-uploader-wrapper {
  position: relative;
  border-radius: 50%;
  overflow: hidden;
}

.face-uploader-wrapper.editing {
  cursor: pointer;
}
